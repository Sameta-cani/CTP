...
def solution(dice: list) -> list:
    ...
    for A_indices in combinations(range(len(dice)), n):
        B_indices = [idx for idx in range(len(dice)) if idx not in A_indices]
        
        A_dice = [dice[idx] for idx in A_indices]
        B_dice = [dice[idx] for idx in B_indices]
    ...
    ans = [idx + 1 for idx in best_combination]
    return ans